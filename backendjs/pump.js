    ...
    sock.on('data', function(data) 
	{
		...
		if(d != ""){
			...
			/* =============['METER_TYPE'] == 'ELM'================ */
				...
				fx += lx; lx = 4;				
				var v2 = d.substr(fx, lx);				
				PF_AVERAGE = getHexToDecimal2(v1,v2);
				...
				console.log("FLOW_TOTAL: "+FLOW_TOTAL);	
				...
				console.log('PF_AVERAGE:',PF_AVERAGE);
...
var level_server_2g = net.createServer(function(sock) {
    ...
    console.log('-----* 2G Level Sensor Data *-----');
    sock.on('data', function(data) {
		var dt = new Date();
...
var level_server_4g = net.createServer(function(sock) {
    ...
    sock.on('data', function(data) {
		// 4G
		console.log('-----* 4G Level Sensor Data *-----');
		var dt = new Date();
		var buff = Buffer.from(data, 'utf8');
		var hexdata = buff.toString('hex');
