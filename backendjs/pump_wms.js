...
var server = net.createServer(function(sock) {
    ...
    sock.on('data', function(data) {
		...
		//if(IMEI_NO=='861190056526632'){
		if(IMEI_NO=='864502039198080'){
		console.log('MYIP== MYIP===========MYIP=========MYIP======MYIP=======MYIP======MYIP===============Ime:'+IMEI_NO+'------'+data);
		}
		console.log('========================================================Ime:'+IMEI_NO+'------'+data);
		...
		console.log('amp_r:', AMP_R,'--amp_y:',AMP_Y,'--amp_b:',AMP_B);
		...
		var end_time = moment(moment().format('YYYY-MM-DD')+' 23:59:59').format('YYYY-MM-DD hh:mm:ss');
		var diff = current_time.diff(end_time, 'seconds');

		var sql = 'SELECT * FROM report_operation WHERE imei_no = "'+IMEI_NO+'" AND insert_time = "'+moment(end_time).format('YYYY-MM-DD')+'"';
		//console.log(sql);
		connection.query(sql, function selectCb(err, results, fields) {
			if (err){
				console.log("ERROR DATA LENGTH "+data.length);
				console.log("ERROR IMEI_NO "+IMEI_NO);
				console.log(err);
			}else{ 
				var ct = moment(current_time).format('YYYY-MM-DD hh:mm:ss');

				if(results.length > 0){
					
					var seconds_diff = parseInt(current_time.diff(results[0].update_time, 'seconds'));
					var up_time = results[0].total_up_time;
					var down_time = results[0].total_down_time;

					var power_up_time = results[0].power_up_time;
					var power_down_time = results[0].power_down_time;


					console.log(LAST_AMP+" "+AMP+" "+seconds_diff+" "+results[0].total_up_time+" "+results[0].total_down_time);

					if(LAST_AMP > 0 && AMP > 0 && seconds_diff < 300){
						var tut = results[0].total_up_time.split(":");

						tut[0] = parseInt(tut[0]);
						tut[1] = parseInt(tut[1]);
						tut[2] = parseInt(tut[2]);

						var carry2 = parseInt((tut[2] + seconds_diff) / 60);
						tut[2] = (tut[2] + seconds_diff) % 60;
						console.log(">TUT "+tut[2]+" "+carry2);
						
						var carry1 = parseInt((tut[1] + carry2) / 60);
						tut[1] = (tut[1] + carry2) % 60;
						console.log(">TUT "+tut[1]+" "+carry1);
						
						var carry0 = parseInt((tut[0] + carry1) / 60);
						tut[0] = (tut[0] + carry1);
						console.log(">TUT "+tut[0]+" "+carry0);

						up_time = tut[0]+":"+tut[1]+":"+tut[2];
					}else{
						var tdt = results[0].total_down_time.split(":");

						tdt[0] = parseInt(tdt[0]);
						tdt[1] = parseInt(tdt[1]);
						tdt[2] = parseInt(tdt[2]);

						var carry2 = parseInt((tdt[2] + seconds_diff) / 60);
						tdt[2] = (tdt[2] + seconds_diff) % 60;
						console.log(">TDT "+tdt[2]+" "+carry2);
						
						var carry1 = parseInt((tdt[1] + carry2) / 60);
						tdt[1] = (tdt[1] + carry2) % 60;
						console.log("> "+tdt[1]+" "+carry1);

						var carry0 = parseInt((tdt[0] + carry1) / 60);
						tdt[0] = (tdt[0] + carry1);
						console.log("> "+tdt[0]+" "+carry0);

						down_time = tdt[0]+":"+tdt[1]+":"+tdt[2];
						
					}

					if(PREV_VOLTAGE > 0 && VOLTAGE > 0 && seconds_diff < 300){
						var tut = "";
						if(results[0].power_up_time != null){
							tut = results[0].power_up_time.split(":");
						}else{
							tut = String("0:0:0").split("0");;
						}
						...
						power_up_time = tut[0]+":"+tut[1]+":"+tut[2];
					}else{
						var tdt = "";
						if(results[0].power_down_time != null){
							tdt = results[0].power_down_time.split(":");
						}else{
							tdt = String("0:0:0").split("0");
						}
						...
						power_down_time = tdt[0]+":"+tdt[1]+":"+tdt[2];
						
					}
					connection.query('UPDATE device_performance SET total_up_time = "'+up_time+'", total_down_time = "'+down_time+'", power_up_time = "'+power_up_time+'", power_down_time = "'+power_down_time+'", last_time = NOW(), update_time = NOW() WHERE imei_no = "'+IMEI_NO+'" AND insert_time = "'+moment(end_time).format('YYYY-MM-DD')+'"', function selectCb(err, results, fields) {
						if (err) {
							console.log(err);
						}else{ 
							console.log("UPDATE DEVICE PERFORMANCE OK");
						}
					});
				}else{
					connection.query("INSERT INTO device_performance (imei_no, total_up_time, total_down_time, power_up_time, power_down_time, last_value, 	last_time, insert_time, update_time) VALUES ('"+IMEI_NO+"', '00:00:00', '00:00:00', '00:00:00', '00:00:00', 0, '"+ct+"', NOW(), NOW())", function selectCb(err, results, fields) {
						if (err) {
							console.log(err);
						}else{ 
							console.log("INSERT DEVICE PERFORMANCE OK");
						}
					});
				}
			}
...
/*var jodhpurserver = net.createServer(function(sock) {
    console.log('SERVER CON: ' + sock.remoteAddress +':'+ sock.remotePort);
    ...
    sock.on('data', function(data) {
		var dt = new Date();
		console.log('JODHPUR DATA RECIEVED ' + sock.remoteAddress + ': ' + data +' '+(dt.getTime()-ST));
		if(data.length < 15){
			return;
		}
		writeLog(IMEI_NO, data);
...
jodhpurserver.listen(PORT2, HOST);
jodhpurserver.on('connection', function(sock){
	console.log('JODHPUR SOCKET CONNECTION: ' + sock.remoteAddress +':'+ sock.remotePort);
	sockets.push(sock);
	//sock.pipe(sock);
	...
	fs.exists(path.normalize(__dirname+'../../../../logs/'+file), function(exist){
		if(exist){
			...
			fs.writeFile(path.normalize(__dirname+'../../../../logs/'+file), '--- Start Logging ---', function(err2) {
				...
				fs.chown(path.normalize(__dirname+'../../../../logs/'+file), 1005, 1005, (error) => { 
				  if (error) 
