...
function load_levels(){
	...
	var sql = "SELECT imei_no, tank_height, sensor_height, level_min, level_max FROM master_location WHERE imei_no IN (SELECT ml.imei_no FROM master_pumps mp LEFT JOIN master_location ml ON mp.tank_imei_no = ml.imei_no WHERE mp.tank_imei_no IS NOT NULL GROUP BY ml.imei_no);";
	connection.query(sql, function selectCb(err, results, fields) {
		if (err) console.log(err);
		else {
			if(results.length > 0){
