...
"if(drd.supply>120, 'BATTERY', 'MAIN'), "+
"if(drd.voltage_r < 8000,0,drd.voltage_r/100), "+
"if(drd.voltage_y < 8000,0,drd.voltage_y/100), "+
"if(drd.voltage_b < 8000,0,drd.voltage_b/100), "+
"CASE WHEN drd.meter_phase_type IS NOT NULL THEN drd.amp_r/100 ELSE if(drd.output_status=0 AND (drd.amp_r/1000 < 2.3), if(drd.voltage_r < 8000,0,drd.amp_r/1000), if(drd.voltage_r < 8000,0,drd.amp_r/1000) END, "+
"CASE WHEN drd.meter_phase_type IS NOT NULL THEN drd.amp_y/100 ELSE if(drd.output_status=0 AND (drd.amp_y/1000 < 2.3), if(drd.voltage_y < 8000,0,drd.amp_y/1000), if(drd.voltage_y < 8000,0,drd.amp_y/1000)) END, "+
"CASE WHEN drd.meter_phase_type IS NOT NULL THEN drd.amp_b/100 ELSE if(drd.output_status=0 AND (drd.amp_b/1000 < 2.3), if(drd.voltage_b < 8000,0,drd.amp_b/1000), if(drd.voltage_b < 8000,0,drd.amp_b/1000)) END, "+
"CASE WHEN drd.meter_phase_type IS NOT NULL THEN drd.kw_r/1000 ELSE if(drd.output_status=0 AND (drd.amp_r/1000 < 2.3 OR drd.amp_y/1000 < 2.3 OR drd.amp_b/1000 < 2.3), if(drd.voltage_r < 8000,0,drd.kw_r/100000), if(drd.voltage_r < 8000,0,drd.kw_r/100000)) END, "+
"CASE WHEN drd.meter_phase_type IS NOT NULL THEN drd.kw_y/1000 ELSE if(drd.output_status=0 AND (drd.amp_r/1000 < 2.3 OR drd.amp_y/1000 < 2.3 OR drd.amp_b/1000 < 2.3), if(drd.voltage_y < 8000,0,drd.kw_y/100000), if(drd.voltage_y < 8000,0,drd.kw_y/100000)) END, "+
"CASE WHEN drd.meter_phase_type IS NOT NULL THEN drd.kw_b/1000 ELSE if(drd.output_status=0 AND (drd.amp_r/1000 < 2.3 OR drd.amp_y/1000 < 2.3 OR drd.amp_b/1000 < 2.3), if(drd.voltage_b < 8000,0,drd.kw_b/100000), if(drd.voltage_b < 8000,0,drd.kw_b/100000)) END, "+
"drd.kwh/100, "+
"CASE WHEN drd.meter_phase_type IS NOT NULL THEN drd.total_kw/1000 ELSE if(drd.output_status=0 AND (drd.amp_r/1000 < 2.3 OR drd.amp_y/1000 < 2.3 OR drd.amp_b/1000 < 2.3), 0, drd.total_kw/100000) END, "+
"drd.update_time, if(drd.door_status=1, 'OPENED', 'CLOSED'), "+
"if(drd.output_status=1, 'ON', 'OFF') "+
"FROM device_received_data_clone drd LEFT JOIN master_device md ON drd.imei_no=md.imei_no WHERE md.feeder_pillar_no='"+insertdata.feeder_pillar_no+"' AND drd.data_stamp >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND drd.data_stamp <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') ORDER BY drd.id DESC LIMIT 300 INTO OUTFILE '"+mysqlpath+"' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';"
...
router.post('/api/reports-generate', checkAuth, function(req, res, next) {
	...
	if(type == "OVERVOLTAGE"){
		...
		var sql = "SELECT 'DISTRICT', 'LOCATION', 'FEEDER PILLAR', 'TOTAL LOAD (KW)','START TIME','START LOAD (KW)','END TIME','END LOAD (KW)' UNION ALL SELECT mz2.title, REPLACE(ml.location, ',', ''), ov.fp_no, (ml.total_load/1000), ov.start_time, (ov.start_load/100000), ov.end_time, (ov.end_load/100000) FROM report_overload ov LEFT JOIN master_zone mz ON ov.ulb=mz.id LEFT JOIN master_zone mz2 ON ov.district=mz2.id LEFT JOIN master_location ml ON ml.feeder_pillar_no = ov.fp_no WHERE ml.total_load < (ov.start_load/100) AND ov.start_time >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.end_time <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') AND mz.id='"+insertdata.ulb+"' ORDER BY ov.id DESC LIMIT 300 INTO OUTFILE '"+mysqlpath+"' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';";
		...
		new Promise((resolve, reject) => {
            ...
            console.log('START MOVING FILE '+result);

           	generatePdf(mysqlpath, filepath, filepathpdf, function(){
