...
router.post('/api/dashboard', checkAuth, function(req, res, next) {
	...
	if(!empty(perms.state_perm)){
		sql = "SELECT IFNULL(dld.supply, '') as supply, IFNULL(dld.update_time, '') as update_time FROM master_device as md LEFT JOIN device_last_data as dld ON dld.imei_no = md.imei_no LEFT JOIN master_location ml ON ml.feeder_pillar_no=md.feeder_pillar_no WHERE ml.state_id='"+perms.state_perm+"';";
		...
		sql += "SELECT ru.ulb_id, mz.state as state_id, mz.parent as zone_id, ru.total_device, ru.total_device_loss, ru.total_controller, ru.total_controller_on, ru.total_controller_off, ru.total_controller_loss, ru.total_powercut, ru.total_load, ru.actual_load, ru.total_kwh, mz.title as ulb_title, mz.areacode ulb_areacode, ru.update_time, mz2.title zone_title, mz2.areacode zone_areacode FROM report_ulb ru LEFT JOIN master_zone mz ON ru.ulb_id=mz.id LEFT JOIN master_zone mz2 ON mz.parent = mz2.id WHERE mz2.state='"+perms.state_perm+"';";
		sql += "SELECT id, title, areacode FROM master_zone WHERE parent=0 AND state='"+perms.state_perm+"';";
	}else{
...
router.post('/api/reports-generate', checkAuth, function(req, res, next) {
	...
	if(type == "OVERVOLTAGE"){
		...
		var sql="";
		if(insertdata.ulb != ''){
			sql = "SELECT 'ULB', 'DEVICES','DEVICES LOSS','CONTROLLERS','CONTROLLER ON','CONTROLLER OFF','CONTROLLER LOSS', 'POWER CUT', 'TOTAL LOAD', 'ACTUAL LOAD', 'TOTAL KWH', 'UPDATE TIME' UNION ALL SELECT mz.title, ov.total_device, ov.total_device_loss, ov.total_controller, ov.total_controller_on, ov.total_controller_off, ov.total_controller_loss, ov.total_powercut, ov.total_load, ov.actual_load, ov.total_kwh, ov.update_time FROM report_ulb_data ov LEFT JOIN master_zone mz ON ov.ulb_id=mz.id WHERE ov.update_time >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.update_time <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') AND ov.ulb_id='"+insertdata.ulb+"' INTO OUTFILE '"+filepath+"' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';";
		}else{
			sql = "SELECT 'ULB', 'DEVICES','DEVICES LOSS','CONTROLLERS','CONTROLLER ON','CONTROLLER OFF','CONTROLLER LOSS', 'POWER CUT', 'TOTAL LOAD', 'ACTUAL LOAD', 'TOTAL KWH', 'UPDATE TIME' UNION ALL SELECT mz.title, ov.total_device, ov.total_device_loss, ov.total_controller, ov.total_controller_on, ov.total_controller_off, ov.total_controller_loss, ov.total_powercut, ov.total_load, ov.actual_load, ov.total_kwh, ov.update_time FROM report_ulb_data ov LEFT JOIN master_zone mz ON ov.ulb_id=mz.id WHERE ov.update_time >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.update_time <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') AND mz.state='"+state_perm+"' INTO OUTFILE '"+filepath+"' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';";
		}
		var ulb = '';
		conn.query(sql, function (err, results, fields) {
			if (err) {
