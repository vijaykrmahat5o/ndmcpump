...
router.post('/api/device-insert', checkAuth, async function(req, res, next) {
	...
	if(insertdata.imei_no != ''){
		var device_id = await new Promise((resolve, reject) => {
			pool.getConnection(function(err, connection) {
				if (err){
					...
					var q = connection.query(sql, sqlquery, function (error, results, fields) {
						console.log(q.sql);
						if (error) {
							resolve(0);
						}else{
		...
		var location_id = await new Promise((resolve, reject) => {
			pool.getConnection(function(err, connection) {
				if (err){
					console.log(err);
					resolve(0);
					...
					var sql = "INSERT INTO master_location SET ?, insert_time = NOW());";
					var q = connection.query(sql, sqlquery, function (error, results, fields) {
						console.log(q.sql);
						if (error) {
							resolve(0);
		...
		var lastdata_id = await new Promise((resolve, reject) => {
			pool.getConnection(function(err, connection) {
				if (err){
					console.log(err);
					resolve(0);
				}else{
					var sql = "INSERT INTO device_last_data (imei_no) VALUES ('"+insertdata.imei_no+"');";
		...
		if(device_id == 0 || location_id > 0 && lastdata_id > 0){
