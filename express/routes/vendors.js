...
var vendorpool = mysql.createPool({
  connectionLimit: 100,
  host: "localhost",
  user: "ccms_usr",
  password: "ah8vx61mxrbv",
  database: "ccms_vendordb",
...
var pool_rj = mysql.createPool({
  connectionLimit: 50,
  host: "localhost",
  user: "ccmsrj_usr",
  password: "883kpy86va7f",
  database: "ccmsrj_db",
  multipleStatements: true,
  dateStrings: ["DATE", "DATETIME"],
});
var pool_dl = mysql.createPool({
  connectionLimit: 50,
  host: "localhost",
  user: "ccmsdl_usr",
  password: "19o13rw00y62",
  database: "ccmsdl_db",
  multipleStatements: true,
  dateStrings: ["DATE", "DATETIME"],
});
...
var POOLDB = {
  ...
  "216.10.245.169": pool_rj,
  "119.18.52.29": pool_dl,
  "mh.ccms.live": pool,
};
var DASHBOARD = {
  "216.10.251.190": "/state-dashboard/in-mh",
  "103.211.218.36": "/state-dashboard/in-up",
  "216.10.245.169": "/state-dashboard/in-rj",
  "119.18.52.29": "/state-dashboard/in-dl",
  "mh.ccms.live": "/state-dashboard/in-mh",
};
var STATEIP = {
...
router.get("/", function (req, res, next) {
  ...
  res.render("index", { title: "Express Fzzzzzzzzzzzzzzz" });
});

console.log("----------------Vandars API---------------------::::");
...
function checkAuth(req, res, next) {
  ...
  if (!accesskey) {
    ...
    vendorpool.getConnection(function (err, connection) {
      if (err) console.log(err);

      var sql = 'SELECT u.id, u.vid, u.title, u.username FROM users u WHERE u.access_token = "' + accesskey + '";';
      connection.query(sql, function (error, result) {
        if (error) {
          console.log(error);
          connection.release();
...
router.post("/api/get-feeder-pillar", checkAuth, function (req, res, next) {
  ...
  if (fp_no > 0) {
    POOLDB[eval(state)].getConnection(function (err, connection) {
      ...
      var sql =
        'SELECT imei_no, location, state_id, ulb_id FROM master_location WHERE feeder_pillar_no="' + fp_no + '"';
...
router.post("/dashboard", checkAuth, function (req, res, next) {
  console.log("VID: " + req.vid);
  vendorpool.getConnection(function (err, connection) {
    if (err) console.log(err);

    var sql = "";
    if (req.vid == "EESL") {
      /*sql += "SELECT uld.ulb_id, uld.vid,  mu.previous_load, mu.tariff_rate, uld.total_device, uld.total_device_on, uld.total_device_off, uld.total_device_loss, uld.total_controller, uld.total_controller_on, uld.total_controller_off, uld.total_controller_loss, uld.total_powercut, uld.total_manual_on, uld.total_load, uld.actual_load, uld.total_kwh, mu.title as ulb_title, mu.areacode ulb_areacode, uld.update_time, mu.title zone_title, mu.areacode zone_areacode FROM ulb_last_data uld LEFT JOIN master_ulbs mu ON uld.ulb_id=mu.ulb_id WHERE mu.vid IS NOT NULL AND uld.vid IS NOT NULL;";
			sql += "SELECT mu.id, mu.title, mu.areacode FROM master_ulbs mu;";*/
      //sql += "SELECT uld.ulb_id, uld.vid, city_code,  mu.previous_load, mu.tariff_rate, uld.total_device, uld.total_device_on, uld.total_device_off, uld.total_device_loss, uld.total_controller, uld.total_controller_on, uld.total_controller_off, uld.total_controller_loss, uld.total_powercut, uld.total_manual_on, uld.total_load, uld.actual_load, uld.total_kwh, mu.title as ulb_title, mu.areacode ulb_areacode, uld.update_time, mu.title zone_title, mu.areacode zone_areacode FROM ulb_last_data uld LEFT JOIN master_ulbs mu ON mu.ulb_id = uld.ulb_id AND mu.vid = uld.vid WHERE mu.ulb_id IS NOT NULL AND mu.vid IS NOT NULL; ";
      sql +=
        "SELECT uld.ulb_id, uld.vid, d.areacode city_code, d.title city_title,  mu.previous_load, mu.tariff_rate, uld.total_device, uld.total_device_on, uld.total_device_off, uld.total_device_loss, uld.total_controller, uld.total_controller_on, uld.total_controller_off, uld.total_controller_loss, uld.total_powercut, uld.total_manual_on, uld.total_load, uld.actual_load, uld.total_kwh, mu.title as ulb_title, mu.areacode ulb_areacode, uld.update_time, mu.title zone_title, mu.areacode zone_areacode FROM ulb_last_data uld LEFT JOIN master_ulbs mu ON mu.ulb_id = uld.ulb_id AND mu.vid = uld.vid LEFT JOIN district d ON d.id = mu.district WHERE mu.ulb_id IS NOT NULL AND mu.vid IS NOT NULL; ";
      sql +=
        "SELECT mu.id, mu.title, mu.areacode, mu.district, d.title city_title, d.areacode city_code FROM master_ulbs mu LEFT JOIN district d ON d.id = mu.district WHERE mu.vid IS NOT NULL AND mu.ulb_id IS NOT NULL;";
    } else {
      sql +=
        "SELECT uld.ulb_id, uld.vid, d.areacode city_code, d.title city_title,  mu.previous_load, mu.tariff_rate, uld.total_device, uld.total_device_on, uld.total_device_off, uld.total_device_loss, uld.total_controller, uld.total_controller_on, uld.total_controller_off, uld.total_controller_loss, uld.total_powercut, uld.total_manual_on, uld.total_load, uld.actual_load, uld.total_kwh, mu.title as ulb_title, mu.areacode ulb_areacode, uld.update_time, mu.title zone_title, mu.areacode zone_areacode FROM ulb_last_data uld LEFT JOIN master_ulbs mu ON uld.ulb_id=mu.ulb_id LEFT JOIN district d ON d.id = mu.district WHERE mu.vid='" +
        req.vid +
        "' AND uld.vid='" +
        req.vid +
        "';";
      //sql += "SELECT uld.ulb_id, mu.previous_load, mu.tariff_rate, SUM(uld.total_device) total_device, SUM(uld.total_device_on) total_device_on, uld.total_device_off, uld.total_device_loss, uld.total_controller, uld.total_controller_on, uld.total_controller_off, uld.total_controller_loss, uld.total_powercut, uld.total_manual_on, uld.total_load, uld.actual_load, uld.total_kwh, mu.title as ulb_title, mu.areacode ulb_areacode, uld.update_time, mu.title zone_title, mu.areacode zone_areacode FROM ulb_last_data uld LEFT JOIN master_ulbs mu ON uld.ulb_id=mu.ulb_id WHERE uld.vid = '"+req.vid+"';";
      sql +=
        "SELECT mu.id, mu.title, mu.areacode, mu.district, d.title city_title, d.areacode city_code FROM master_ulbs mu LEFT JOIN district d ON d.id = mu.district WHERE mu.vid='" +
        req.vid +
        "';";
    }
    ...
    connection.query(sql, function (error, result) {
      ...
      var o = { records: result[0], ulbs: result[1] };
      var mess = { error: 0, message: "Master devices!", result: o };
      res.send(JSON.stringify(mess));
    });
  });
...
router.post("/district-record", checkAuth, function (req, res, next) {
  var sql = "SELECT id, state, title, areacode FROM district";
  ...
  vendorpool.getConnection(function (err, connection) {
    if (err) {
      ...
      connection.query(sql, function (error, result) {
        ...
        var mess = { error: 0, message: "District records!", result: result };
        res.send(JSON.stringify(mess));
      });
    }
  });
});
router.post("/district-insert", checkAuth, saveUserActivity, function (req, res, next) {
  ...
  if (insertdata.state != "") {
    ...
    var sql = "INSERT INTO district SET ?";
      ...
      if (err) {
        ...
        connection.query(sql, insertq, function (error, result) {
          ...
          if (error) {
            ...
            var mess = { error: 0, message: "District Added Successfully!", result: 1 };
            res.send(JSON.stringify(mess));
          }
        });
      }
...
router.post("/district-update", checkAuth, saveUserActivity, function (req, res, next) {
  ...
  if (updatedata.id != "") {
    ...
    vendorpool.getConnection(function (err, connection) {
      ...
      var sql = 'UPDATE district SET ? WHERE id="' + updatedata.id + '"';
      ...
      connection.query(sql, updateq, function (error, result) {
        ...
        if (error) {
          ...
          if (err) {
            ...
            var mess = { error: 0, message: "District Updated Successfully!", result: 1 };
            res.send(JSON.stringify(mess));
          }
        }
      });
...
router.post("/district-delete", checkAuth, saveUserActivity, function (req, res, next) {
  var deleteid = req.body.deleteid;
  if (deleteid != "") {
    vendorpool.getConnection(function (err, connection) {
      if (err) console.log(err);

      var sql = "DELETE FROM district WHERE id='" + deleteid + "';";
      ...
      connection.query(sql, updateq, function (error, result) {
        ...
        if (error) {
          ...
          if (err) {
            ...
            var mess = { error: 0, message: "District deleted!", result: 1 };
            res.send(JSON.stringify(mess));
          }
        }
      });
...
router.post("/vendor-ulbs", checkAuth, function (req, res, next) {
  console.log("VID: " + req.vid);
  ...
  if (req.vid == "EESL") {
    sql =
      "SELECT id, vid, ulb_id, district, title, areacode, tariff_rate, previous_load FROM master_ulbs WHERE parent > 0";
  } else {
    sql =
      "SELECT id, vid, ulb_id, district, title, areacode, tariff_rate, previous_load FROM master_ulbs WHERE parent > 0 AND vid = ?";
    params = [req.vid];
  }
  //var sql = "SELECT ulb_id, district, title, areacode, tariff_rate, previous_load FROM master_ulbs WHERE parent > 0 AND vid = ?";
...
router.post("/ulb-list", checkAuth, function (req, res, next) {
  var sql = "SELECT * FROM master_ulbs WHERE parent > 0";
...
router.post("/ulb-record", checkAuth, function (req, res, next) {
  var sql =
...
router.post("/ulb-insert", checkAuth, function (req, res, next) {
  var insertdata = req.body.data;
  if (insertdata.state != "") {
    var insertq = {
      state: insertdata.state,
    ...
    if (!empty(insertdata.logo)) {
      insertq.logo = insertdata.logo;
    }
    if (!empty(insertdata.ulb_id)) {
      insertq.ulb_id = insertdata.ulb_id;
    }
    if (!empty(insertdata.vid)) {
      insertq.vid = insertdata.vid;
    }
    ...
    var sql = "INSERT INTO master_ulbs SET ?, update_time=NOW() ";
      ...
      if (err) {
        ...
        var query = connection.query(sql, insertq, function (error, result) {
          ...
          if (true) {
            ...
            var sql = "INSERT INTO ulb_last_data SET ?, update_time = NOW();";
              ...
              if (err) {
                ...
                connection.query(sql, insertq, function (error, result) {
                  ...
                  if (err) {
                    ...
                    var mess = { error: 0, message: "ULB Added Successfully!", result: 1 };
                    res.send(JSON.stringify(mess));
                  }
                });
              }
            ...
            var mess = { error: 0, message: "Error in insert ULB!", result: 1 };
            res.send(JSON.stringify(mess));
          }
        });
      }
...
router.post("/ulb-update", checkAuth, function (req, res, next) {
  var updatedata = req.body.data;
  if (updatedata.id != "") {
    var updateq = {
    ...
    if (!empty(updatedata.ulb_id)) {
      updateq.ulb_id = updatedata.ulb_id;
    }
    if (!empty(updatedata.logo)) {
      updateq.logo = updatedata.logo;
    }
    if (!empty(updatedata.vid)) {
      updateq.vid = updatedata.vid;
    }
    ...
    vendorpool.getConnection(function (err, connection) {
      ...
      var sql = 'UPDATE master_ulbs SET ? , update_time = NOW() WHERE id="' + updatedata.id + '"';
...
router.post("/api/zone-delete", checkAuth, function (req, res, next) {
  ...
  if (deleteid != "") {
    POOLDB[req.hostname].getConnection(function (err, connection) {
      ...
      var sql =
        "DELETE FROM master_zone WHERE id='" + deleteid + "'; DELETE FROM report_ulb WHERE ulb_id='" + deleteid + "'";
...
router.post("/api/rtc-mismatch", checkAuth, function (req, res, next) {
  var ulb = req.body.ulb;

  var sql = "";
  if (ulb == 0) {
    sql =
      "SELECT ml.feeder_pillar_no, ml.imei_no, ml.mobile_no, REPLACE(ml.location, ',', '') as location, ml.no_of_fittings, ml.insert_time, dld.data_stamp, dld.update_time, TIMESTAMPDIFF(SECOND, dld.data_stamp, dld.update_time) AS rtc_diff FROM master_location ml LEFT JOIN device_last_data dld ON ml.imei_no = dld.imei_no WHERE (TIMESTAMPDIFF(SECOND, dld.data_stamp, dld.update_time) > (60 * 60) OR TIMESTAMPDIFF(SECOND, dld.data_stamp, dld.update_time) IS NULL) AND dld.update_time IS NOT NULL";
  } else {
    sql =
      "SELECT ml.feeder_pillar_no, ml.imei_no, ml.mobile_no, REPLACE(ml.location, ',', '') as location, ml.no_of_fittings, ml.insert_time, dld.data_stamp, dld.update_time, TIMESTAMPDIFF(SECOND, dld.data_stamp, dld.update_time) AS rtc_diff FROM master_location ml LEFT JOIN device_last_data dld ON ml.imei_no = dld.imei_no WHERE (TIMESTAMPDIFF(SECOND, dld.data_stamp, dld.update_time) > (60 * 60) OR TIMESTAMPDIFF(SECOND, dld.data_stamp, dld.update_time) IS NULL) AND dld.update_time IS NOT NULL AND ml.ulb_id=" +
      ulb;
  }
  ...
  POOLDB[req.hostname].getConnection(function (err, connection) {
    ...
    connection.query(sql, function (error, result) {
      ...
      if (error) {
        ...
        o.records = result;
        o.time = new Date();
        var mess = { error: 0, message: "RTC Records!", result: o };
        res.send(JSON.stringify(mess));
      }
    });
  });
...
router.post("/api/consumption", checkAuth, function (req, res, next) {
  ...
  var sql =
    "SELECT feeder_pillar_no, IFNULL(consumer_no, '') as consumer_no, imei_no, REPLACE(location, ',', '') as location, phase, meter_no, no_of_fittings FROM master_location WHERE ulb_id=" +
    ulb +
    ";";
  sql +=
...
router.post("/api/monthlyconsumption", checkAuth, function (req, res, next) {
  ...
  var sql =
    "SELECT ml.total_load, zo.title as zone, IFNULL(ml.consumer_no, '') as consumer_no, ml.feeder_pillar_no, ml.phase, ml.imei_no, REPLACE(ml.location, ',', '') as location, ml.meter_no, ml.no_of_fittings, ml.led_18w, ml.led_20w, ml.led_24w, ml.led_28w, ml.led_32w, ml.led_35w, ml.led_40w, ml.led_45w, ml.led_50w, ml.led_60w, ml.led_70w, ml.led_72w, ml.led_75w, ml.led_80w, ml.led_90w, ml.led_96w, ml.led_100w, ml.led_110w, ml.led_120w, ml.led_130w, ml.led_140w, ml.led_150w, ml.led_190w, ml.led_200w, ml.led_250w, ml.led_400w FROM master_location ml LEFT JOIN zones as zo ON zo.ulb_id = ml.ulb_id AND zo.id=ml.zone_id WHERE ml.ulb_id=" +
    ulb +
    ";";
...
router.post("/device-ulbs", checkAuth, function (req, res, next) {
  ...
  //}else{
  sql = "SELECT * FROM master_ulbs WHERE areacode='" + ulbcode + "';";
  //}
  var moment = require("moment");
  ...
  vendorpool.getConnection(function (err, connection) {
    ...
    connection.query(sql, function (error, result) {
      ...
      if (error) {
        ...
        var ulb_id = result[0]["ulb_id"];
        var ulb_vid = result[0]["vid"];
        var zoneinfo = result[0];

        var dc_diff = moment().diff(moment(moment().format("YYYY-MM-DD 12:00:00")));
        var start_date = "";
        if (dc_diff < 0) {
          // below 12 clock
          start_date = moment().subtract(1, "day").format("YYYY-MM-DD 12:00:00");
        } else {
          // above 12 clock
          start_date = moment().format("YYYY-MM-DD 12:00:00");
        }
        ...
        if (req.vid == "EESL") {
          sql =
            "SELECT ml.id, ml.imei_no, ml.fp_no, dld.door_status, ml.ulb_id, ml.phase, ml.physical_load, ml.latitude, ml.longitude, ml.no_of_poles, ml.location, COALESCE(dld.total_kw, 0) total_kw, dld.datalength, dld.supply, dld.amp_r, dld.amp_y, dld.amp_b, dld.kw_r, dld.kw_y, dld.kw_b, dld.kwh, ml.led_fittings, dld.output_status, DATE_FORMAT(dld.data_stamp, '%Y-%m-%d %H:%i:%s') as data_stamp, DATE_FORMAT(dld.data_stamp, '%Y-%m-%d %H:%i:%s') as update_time, DATE_FORMAT(ml.insert_time, '%Y-%m-%d %H:%i:%s') as insert_time " +
            "FROM locations AS ml " +
            "LEFT JOIN last_data AS dld ON dld.imei_no = ml.imei_no " +
            "WHERE ml.ulb_id=" +
            ulb_id +
            " AND ml.vid = '" +
            ulb_vid +
          ...
          sql +=
            "SELECT min(kwh) minkwh, max(kwh) maxkwh FROM daily_consumption WHERE insert_time > '" +
            start_date +
            "' AND vid='" +
            ulb_vid +
            "' AND ulb_id=" +
        ...
        vendorpool.getConnection(function (err, connection) {
          if (err) {
            ...
            connection.query(sql, function (error, result) {
              ...
              var o = {};
              o.devices = result[0];
              o.consumption = result[1][0];
              o.ulbs = zoneinfo;
              o.time = new Date();
              var mess = { error: 0, message: "Ulb devices!", result: o, s: sql };
              res.send(JSON.stringify(mess));
            });
          }
        });
    ...
    POOLDB[req.hostname].getConnection(function (err, connection) {
      ...
      connection.query(sql, [insertdata.imei_no], function (error, result) {
        ...
        if (result.length > 0) {
          ...
          if (!empty(insertdata.led_fittings)) {
            location.led_fittings = JSON.stringify(insertdata.led_fittings);
          } else {
            location.led_fittings = "{}";
          }
...
router.post("/api/device-update", checkAuth, saveUserActivity, function (req, res, next) {
  ...
  if (updatedata.imei_no != "") {
    ...
    if (!empty(updatedata.led_fittings)) {
      location.led_fittings = JSON.stringify(updatedata.led_fittings);
    } else {
      location.led_fittings = "{}";
    }
    ...
    sql +=
      "UPDATE master_location SET ?, update_time=NOW() WHERE feeder_pillar_no='" + updatedata.feeder_pillar_no + "';";
  ...
  if (!req.files || Object.keys(req.files).length === 0) {
    ...
    if (!empty(csvfile)) {
      ...
      fs.createReadStream(csvfile.tempFilePath)
        ...
        .on("data", (data) => {
          console.log(
            ">>>>" + data.feeder_pillar_no + " " + data.ulb_id + " " + data.imei_no.toString() + " " + data.mobile_no
          );
      ...
      if (err) {
        ...
        connection.query(sql, inputarr, function (error, result) {
          if (error) {
            console.log(error);
            connection.release();
            import_result.push({ fpno: data.feeder_pillar_no, imei_no: data.imei_no, status: "Connection Error" });
            updatedevice(req, res);
          }
          if (result.length > 0) {
            var location = {};
            ...
            if (!empty(data.led_fittings)) {
              location.led_fittings = JSON.stringify(data.led_fittings);
            }
            ...
            /*if(!empty(data.led_18w)){
							...
							if(!empty(data.led_30w)){
								location.led_30w= data.led_30w;
							}
            ...
            if (!empty(data.imei_no) && !empty(data.feeder_pillar_no)) {
              ...
              sql = 'UPDATE master_location SET ? , update_time = NOW() WHERE imei_no="' + data.imei_no + '"';
            }

            connection.query(sql, [location], function (err, result) {
              connection.release();
              if (err) {
                import_result.push({ fpno: data.feeder_pillar_no, imei_no: data.imei_no, status: "Update Error" });
              } else {
                import_result.push({
                  fpno: data.feeder_pillar_no,
                  imei_no: data.imei_no,
...
router.post("/ccms-report", checkAuth, function (req, res, next) {
  var data = req.body.data;
  var condition = data.imei_no;
  if (data.imei_no != "" || data.fp_no != "") {
    vendorpool.getConnection(function (err, connection) {
      if (err) console.log(err);

      var sql =
        'SELECT datalength, IF(supply<1, "OFF", "ON") supply, voltage_r, voltage_y, voltage_b, amp_r, amp_y, amp_b, kw_r, kw_y, kw_b, kwh, total_kw, data_stamp, insert_time as update_time, door_status, output_status FROM received_data WHERE imei_no=? AND insert_time>=? AND insert_time<=? ORDER BY id DESC LIMIT 3000;';

      if (data.fp_no != "") {
        condition = data.fp_no;
        sql =
          'SELECT datalength, IF(supply<1, "OFF", "ON") supply, voltage_r, voltage_y, voltage_b, amp_r, amp_y, amp_b, kw_r, kw_y, kw_b, kwh, total_kw, data_stamp, insert_time as update_time, door_status, output_status FROM received_data WHERE imei_no=(SELECT imei_no FROM locations WHERE fp_no=?) AND insert_time>=? AND insert_time<=? ORDER BY id DESC LIMIT 3000;';
      }
...
router.post("/login", function (req, res, next) {
  ...
  vendorpool.getConnection(function (pool_err, connection) {
    if (pool_err) console.log(pool_err);

    var sql = 'SELECT * FROM users us WHERE us.username="' + email + '" AND us.password="' + pass + '"';
    ...
    connection.query(sql, function (usr_err, usr_result) {
      ...
      if (usr_err) {
        ...
        if (usr_result.length > 0) {
          ...
          var o = {};
          o.access_key = usr_result[0].access_token;
          o.vid = usr_result[0].vid;
          o.user_id = usr_result[0].id;
          o.username = usr_result[0].username;
          o.title = usr_result[0].title;
          o.logo_left = usr_result[0].logo_left;
          o.logo_center = usr_result[0].logo_center;
          o.logo_right = usr_result[0].logo_right;
          o.logo_right = usr_result[0].logo_right;
          o.total_ccms = usr_result[0].total_ccms;
          o.installed_ccms = usr_result[0].installed_ccms;
...
router.post("/api/dailyreports-generate", checkAuth, function (req, res, next) {
  ...
  if (insertdata.imei_no != "") {
    ...
    var filename =
      "report-imei-" + insertdata.imei_no + "-" + insertdata.start_date + "-" + insertdata.end_date + "-" + unique_id;
          ...
          getImeiFromFeederPillar(req, insertdata.feeder_pillar_no, function (imei_no) {
            ...
            if (imei_no > 0) {
              var sql =
                ...
                " 00:00:00', '%y-%m-%d %H:%i:%s') AND data_stamp <= DATE_FORMAT('" +
                insertdata.end_date +
                " 23:59:59', '%y-%m-%d %H:%i:%s') ORDER BY id DESC LIMIT 300 ;";
...
router.post("/ulb-report", checkAuth, function (req, res, next) {
  var user_id = req.user_id;
  var data = req.body.data;
  ...
  vendorpool.getConnection(function (err, connection) {
    ...
    var sql =
      "SELECT ov.total_device, ov.total_device_on, ov.total_device_off, ov.total_device_loss, ov.total_controller, ov.total_controller_on, ov.total_controller_off, ov.total_controller_loss, ov.total_powercut, ov.total_load/1000 total_load, ov.actual_load, ov.total_kwh, ov.total_manual_on, ov.total_underload, ov.total_overload, ov.total_underload_kw, ov.total_overload_kw, ov.update_time FROM ulb_data ov WHERE ov.update_time >= DATE_FORMAT('" +
      data.start_date +
      " 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.update_time <= DATE_FORMAT('" +
      data.end_date +
      " 23:59:59', '%y-%m-%d %H:%i:%s') AND ov.ulb_id=(SELECT ulb_id FROM master_ulbs WHERE id=?) AND ov.vid=(SELECT vid FROM master_ulbs WHERE id=?) GROUP BY hour(ov.update_time) , day( ov.update_time ) ORDER BY ov.id DESC LIMIT 500;";

    sql += "SELECT * FROM master_ulbs WHERE id = ?;";

    connection.query(sql, [data.ulb, data.ulb, data.ulb], function (error, result) {
      connection.release();

      if (error) {
        console.log(error);
      }
      var o = { records: result[0], zone: result[1][0] };
      var mess = { error: 0, message: "ULB Records!", result: o };
      res.send(JSON.stringify(mess));
    });
  });
});
...
router.post("/api/replacepanel-insert", checkAuth, function (req, res, next) {
  ...
  sql += "UPDATE master_device SET ? WHERE feeder_pillar_no=?;";
  sql += "UPDATE master_location SET ? WHERE feeder_pillar_no=?;";

  if (!empty(insertdata.new_fp_no)) {
    insertq.new_feeder_pillar_no = insertdata.new_fp_no;

    update_md.feeder_pillar_no = insertdata.new_fp_no;
    update_ml.feeder_pillar_no = insertdata.new_fp_no;

    //sql += 'UPDATE master_device SET feeder_pillar_no = "'+insertdata.new_fp_no+'" WHERE feeder_pillar_no="'+insertdata.feeder_pillar_no+'";';
    //sql += 'UPDATE master_location SET feeder_pillar_no = "'+insertdata.new_fp_no+'" WHERE feeder_pillar_no="'+insertdata.feeder_pillar_no+'";';
  }
  ...
  if (!empty(insertdata.new_imei_no)) {
    ...
    update_md.imei_no = insertdata.new_imei_no;
    update_ml.imei_no = insertdata.new_imei_no;
  ...
  if (!empty(insertdata.new_meter_no)) {
    ...
    if (!empty(insertdata.replace_meter_no)) {
      ...
      update_md.device_no = insertdata.new_meter_no;
      update_ml.meter_no = insertdata.new_meter_no;
...
router.post("/api/device-oridata", checkAuth, function (req, res, next) {
  var imei_no = req.body.imei_no;
  POOLDB[req.hostname].getConnection(function (err, connection) {
    if (err) console.log(err);

    var sql = "SELECT data FROM device_last_data WHERE imei_no=?";

    connection.query(sql, [imei_no], function (error, result) {
      connection.release();

      if (error) {
        console.log(error);
      }
      var mess = { error: 0, message: "CCMS data!", result: result[0] };
      res.send(JSON.stringify(mess));
    });
  });
});
