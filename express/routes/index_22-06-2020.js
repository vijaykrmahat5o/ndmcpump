...
function checkAuth(req, res, next) {
  if (!req.body.accesskey) {
	...
	res.send(JSON.stringify(mess));
  	...
  	conn.query('SELECT user_id from access_permission WHERE access_key = "'+accesskey+'" AND is_valid=1', function (err, rows, fields) {
	  	if (err) {
	  		console.log(err);
...
router.post('/api/dashboard', checkAuth, function(req, res, next) {
	var sql = "SELECT IFNULL(dld.supply, '') as supply, IFNULL(dld.update_time, '') as update_time FROM master_device as md LEFT JOIN device_last_data as dld ON dld.imei_no = md.imei_no;";
...
var sql = "SELECT md.id, md.imei_no, md.feeder_pillar_no, md.device_type, md.device_no, ml.state_id, ml.zone_id, ml.ulb_id, ml.ward_no, ml.total_connection, ml.total_load, ml.location_lat, ml.location_lng, ml.no_of_fittings, ml.location, COALESCE(dld.total_kw, 0) total_kw, dld.output_status, DATE_FORMAT(dld.data_stamp, '%Y-%m-%d %H:%i:%s') as data_stamp, ml.maintenance, DATE_FORMAT(dld.update_time, '%Y-%m-%d %H:%i:%s') as update_time "+
"FROM master_device AS md "+
"LEFT JOIN master_location AS ml ON ml.feeder_pillar_no = md.feeder_pillar_no "+
"LEFT JOIN master_zone AS mz ON mz.id = ml.ulb_id "+
"LEFT JOIN device_last_data AS dld ON dld.imei_no = md.imei_no "+
"WHERE mz.areacode = '"+ulbcode+"'";
	...
	conn.query(sql, function (err, results, fields) {
	 	...
	 	var mess = {'error':0,'message':'Master devices!','result':results};
		res.send(JSON.stringify(mess));
	})
});
router.post('/api/device-insert', checkAuth, function(req, res, next) {
	var insertdata = req.body.data;
	if(insertdata.imei_no != ''){
		var sql = "INSERT INTO master_device (imei_no, feeder_pillar_no, device_type, device_no, insert_time) VALUES ('"+insertdata.imei_no+"', '"+insertdata.feeder_pillar_no+"', '"+insertdata.device_type+"', '"+insertdata.device_no+"', NOW());";
		...
		sql += "INSERT INTO device_last_data (imei_no, data_stamp, supply, battery, meter_stamp, io_stamp, update_time) VALUES ('"+insertdata.imei_no+"', '0000-00-00 00-00-00', '0', '0', '0000-00-00 00-00-00', '0000-00-00 00-00-00', '0000-00-00 00-00-00');";
...
router.post('/api/device-update', checkAuth, function(req, res, next) {
	...
	if(updatedata.imei_no != ''){
		var sql1 = "UPDATE master_device SET feeder_pillar_no='"+updatedata.feeder_pillar_no+"', device_type='"+updatedata.device_type+"', device_no='"+updatedata.device_no+"' WHERE imei_no='"+updatedata.imei_no+"';";

		var sql2 = "UPDATE master_location SET state_id='"+updatedata.state_id+"', zone_id='"+updatedata.zone_id+"', ulb_id='"+updatedata.ulb_id+"', feeder_pillar_no='"+updatedata.feeder_pillar_no+"', ward_no='"+updatedata.ward_no+"', total_load='"+updatedata.total_load+"', no_of_poles='"+updatedata.no_of_poles+"', no_of_fittings='"+updatedata.no_of_fittings+"', location='"+updatedata.location+"', location_lat='"+updatedata.location_lat+"', location_lng='"+updatedata.location_lng+"' WHERE feeder_pillar_no='"+updatedata.feeder_pillar_no+"';";
...
router.post('/api/zone-record', checkAuth, function(req, res, next) {
	...
	conn.query(sql, function (error, results, fields) {
	  	...
	  	results.forEach(row => { 
			var sql2 = "SELECT id, state, '"+row.title+"' as title, title as ulb, areacode, location, parent, on_time, off_time, status FROM master_zone WHERE parent = "+row.id;
...
router.post('/api/zone-insert', checkAuth, function(req, res, next) {
	var insertdata = req.body.data;
	if(insertdata.state != ''){
		var sql = "INSERT INTO master_zone (state, title, location, areacode, parent, on_time, off_time, status) VALUES ('"+insertdata.state+"', '"+insertdata.title+"', '"+insertdata.location+"', '"+insertdata.areacode+"', '"+insertdata.parent+"', '"+insertdata.on_time+"', '"+insertdata.off_time+"', '"+insertdata.status+"');";
		  	...
		  	if (err) {
				...
				res.send(JSON.stringify(mess));
		  		...
		  		if(insertdata.parent > 0){
		  			var sql = "INSERT INTO report_ulb (ulb_id, state_id, zone_id, update_time) VALUES ('"+results.insertId+"', '"+insertdata.state+"', '"+insertdata.parent+"', NOW());";
...
router.post('/api/zone-update', checkAuth, function(req, res, next) {
	var updatedata = req.body.data;
	if(updatedata.id != ''){
		var sql = "UPDATE master_zone SET state='"+updatedata.state+"', parent='"+updatedata.parent+"', areacode='"+updatedata.areacode+"', title='"+updatedata.title+"', location='"+updatedata.location+"', on_time='"+updatedata.on_time+"', off_time='"+updatedata.off_time+"', status='"+updatedata.status+"' WHERE id='"+updatedata.id+"';";
...
router.post('/api/complaint-record', checkAuth, function(req, res, next) {
	var ulbareacode = req.body.ulbareacode;
	if(ulbareacode != ''){
		var sql = "SELECT * FROM complaints WHERE status=1 ORDER BY id DESC LIMIT 50";
		//console.log(sql);
		conn.query(sql, function (err, results, fields) {
		  	if (err) {
		  		console.log(err);
		  	}
		 	var mess = {'error':0,'message':'Complaint records!','result':results};
			res.send(JSON.stringify(mess));
		})
	}else{
		var mess = {'error':1,'message':'Failed!','result':[]};
...
router.post('/api/dailyreports-records', checkAuth, function(req, res, next) {
	var ulbareacode = req.body.ulbareacode;
	if(ulbareacode != ''){
		var sql = "SELECT * FROM daily_reports ORDER BY id DESC LIMIT 100";
		//console.log(sql);
		conn.query(sql, function (err, results, fields) {
		  	if (err) {
		  		console.log(err);
...
router.post('/api/dailyreports-generate', checkAuth, function(req, res, next) {
	var user_id = req.user_id;
	var insertdata = req.body.data;
	//var date = insertdata.date;
	//var imei_no = insertdata.imei_no;
	//var type = insertdata.type;
	//var feeder_pillar_no = insertdata.feeder_pillar_no;
	const path = require('path');
	//console.log(path.join(__dirname, '../public'));
	if(insertdata.imei_no != ''){
		var unique_id = new Date().getTime();
		
		var filename = 'report-ulb-'+insertdata.imei_no+'-'+insertdata.date+'-'+unique_id+'.csv';
		var tmppath = path.join(__dirname, '../public/exports/'+filename);
		var filepath = tmppath.replace(/\\/g, "/");

		var sql = "SELECT 'SUPPLY','VOLTAGE R','VOLTAGE Y','VOLTAGE B','AMP R','AMP Y','AMP B','KW R','KW Y','KW B','KWH','TOTAL KW','UPDATE TIME','DOOR STATUS','OUTPUT STATUS' UNION ALL SELECT supply, voltage_r, voltage_y, voltage_b, amp_r, amp_y, amp_b, kw_r, kw_y, kw_b, kwh, total_kw, update_time, door_status, output_status FROM device_received_data WHERE imei_no='"+insertdata.imei_no+"' AND data_stamp >= DATE_FORMAT('"+insertdata.date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND data_stamp <= DATE_FORMAT('"+insertdata.date+" 23:59:59', '%y-%m-%d %H:%i:%s') INTO OUTFILE '"+filepath+"' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';";
		console.log(sql);
		conn.query(sql, function (err, results, fields) {
		  	if (err) {
				console.log(err);
		...
		var filename = 'report-ulb-'+insertdata.feeder_pillar_no+'-'+insertdata.date+'-'+unique_id+'.csv';
		var tmppath = path.join(__dirname, '../public/exports/'+filename);
		var filepath = tmppath.replace(/\\/g, "/");

		var sql = "SELECT 'SUPPLY','VOLTAGE R','VOLTAGE Y','VOLTAGE B','AMP R','AMP Y','AMP B','KW R','KW Y','KW B','KWH','TOTAL KW','UPDATE TIME','DOOR STATUS','OUTPUT STATUS' UNION ALL SELECT drd.supply, drd.voltage_r, drd.voltage_y, drd.voltage_b, drd.amp_r, drd.amp_y, drd.amp_b, drd.kw_r, drd.kw_y, drd.kw_b, drd.kwh, drd.total_kw, drd.update_time, drd.door_status, drd.output_status FROM device_received_data drd LEFT JOIN master_device md ON drd.imei_no=md.imei_no WHERE md.feeder_pillar_no='"+insertdata.feeder_pillar_no+"' AND drd.data_stamp >= DATE_FORMAT('"+insertdata.date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND drd.data_stamp <= DATE_FORMAT('"+insertdata.date+" 23:59:59', '%y-%m-%d %H:%i:%s') INTO OUTFILE '"+filepath+"' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';";
		console.log(sql);
		conn.query(sql, function (err, results, fields) {
				if (err) {
				console.log(err);
