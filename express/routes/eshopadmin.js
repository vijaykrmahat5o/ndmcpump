...
router.post('/login', function(req, res, next) {
	...
	pool.getConnection(function(pool_err, connection) {
		...
		var sql = 'SELECT u.id, u.username, u.email, u.role, u.permissions, u.multi_login, ud.dob FROM users u LEFT JOIN user_details ud ON u.id=ud.user_id WHERE (u.email="'+userinfo+'" AND u.password=MD5("'+password+'")) OR (u.mobile_no="'+userinfo+'" AND u.password=MD5("'+password+'"))';
...
router.post('/device-record', checkAuth, function(req, res, next) {
	...
	var sql = sql = "SELECT md.circuit_info, md.device_info, dld.datalength, dld.voltage_r, dld.voltage_y, dld.voltage_b, dld.amp_r, dld.amp_y, dld.amp_b, dld.kw, dld.pf, dld.frequency, dld.kva, dld.kwh, dld.spare_1, dld.spare_2, dld.input_1, dld.input_2, dld.input_3, IFNULL(dld.supply, '') as supply, IFNULL(dld.update_time, '') as update_time FROM master_device as md LEFT JOIN device_last_data as dld ON dld.imei_no = md.imei_no WHERE md.imei_no='"+imeino+"';";
	scada_pool.getConnection(function(err, connection) {
		if (err) console.log(err);
...
router.post('/daily-report', checkAuth, function(req, res, next) {
	...
	var sql = "SELECT md.imei_no, md.title, '19.5 hr' avalibility, '24 hr' network_status, (SELECT count(id) FROM report_trips rt WHERE rt.imei_no=md.imei_no) power_trips, '16 hr' on_hours, dld.datalength, dld.voltage_r, dld.voltage_y, dld.voltage_b, dld.amp_r, dld.amp_y, dld.amp_b, dld.kw, dld.pf, dld.frequency, dld.kva, dld.kwh, dld.spare_1, dld.spare_2, dld.input_1, dld.input_2, dld.input_3, IFNULL(dld.supply, '') as supply, IFNULL(dld.update_time, '') as update_time FROM master_device as md LEFT JOIN device_last_data as dld ON dld.imei_no = md.imei_no;";
	scada_pool.getConnection(function(err, connection) {
		if (err) console.log(err);
		...
		connection.query(sql, function (error, results) {
			connection.release();
		  	...
		  	if (error) {
				...
				var o = {'device': results, 'time': new Date()};
			 	var mess = {'error':0,'message':'Device Record!','result':o};
				res.send(JSON.stringify(mess));
			}
		});
