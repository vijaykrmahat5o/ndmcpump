...
router.post('/api/dailyreports-generate', checkAuth, function(req, res, next) {
	...
	if(insertdata.imei_no != ''){
		...
		var sql = "SELECT 'SUPPLY','VOLTAGE R','VOLTAGE Y','VOLTAGE B','AMP R','AMP Y','AMP B','KW R','KW Y','KW B','KWH','TOTAL KW','UPDATE TIME','DOOR STATUS','OUTPUT STATUS' UNION ALL SELECT if(drd.supply>120, 'BATTERY', 'MAIN'), drd.voltage_r/100, drd.voltage_y/100, drd.voltage_b/100, if(drd.amp_r/1000 < 0.5, 0, drd.amp_r/1000), if(drd.amp_y/1000 < 0.5, 0, drd.amp_y/1000), if(drd.amp_b/1000 < 0.5, 0, drd.amp_b/1000), drd.kw_r/100000, drd.kw_y/100000, drd.kw_b/100000, drd.kwh/100, drd.total_kw/100000, drd.update_time, if(drd.door_status=1, 'OPENED', 'CLOSED'), if(drd.output_status=1, 'ON', 'OFF') FROM device_received_data drd LEFT JOIN master_device md ON drd.imei_no=md.imei_no WHERE md.feeder_pillar_no='"+insertdata.feeder_pillar_no+"' AND drd.data_stamp >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND drd.data_stamp <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') INTO OUTFILE '"+filepath+"' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';";
		//console.log(sql);
		conn.query(sql, function (err, results, fields) {
				if (err) {
				console.log(err);
