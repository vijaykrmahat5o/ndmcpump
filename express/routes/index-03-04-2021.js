		...
		connection.query(sql, function (error, result) {
			...
			var mess = {};
		  	if (error) {
		  		...
		  		if(!empty(perms.state_perm)){
					sql = "SELECT md.id, md.imei_no, md.feeder_pillar_no, md.mobile_no, md.device_type, md.device_no, md.data_fault, dld.meter_no as sys_meter_no, dld.meter_fault, dld.door_status, COALESCE(md.fault_time, '') as fault_time, ml.state_id, ml.zone_id, ml.ulb_id, ml.ward_id, ml.meter_type, ml.total_connection, ml.previous_load, ml.total_load, ml.location_lat, ml.location_lng, ml.no_of_fittings, ml.location, ml.led_18w, ml.led_24w, ml.led_35w, ml.led_45w, ml.led_60w, ml.led_70w, ml.led_75w, ml.led_80w, ml.led_110w, ml.led_130w, ml.led_140w, ml.led_190w, ml.led_200w, COALESCE(dld.total_kw, 0) total_kw, dld.datalength, dld.supply, dld.amp_r, dld.amp_y, dld.amp_b, dld.kw_r, dld.kw_y, dld.kw_b, dld.kwh, dld.output_status, DATE_FORMAT(dld.data_stamp, '%Y-%m-%d %H:%i:%s') as data_stamp, dld.meter_phase_type, ml.maintenance, DATE_FORMAT(dld.update_time, '%Y-%m-%d %H:%i:%s') as update_time, DATE_FORMAT(ml.insert_time, '%Y-%m-%d %H:%i:%s') as insert_time "+
					"FROM master_device AS md "+
					"LEFT JOIN master_location AS ml ON ml.feeder_pillar_no = md.feeder_pillar_no "+
					"LEFT JOIN device_last_data AS dld ON dld.imei_no = md.imei_no "+
		...
		if(data.imei_no == 0){
			...
			POOLDB[req.hostname].getConnection(function(err, connection) {
				if (err){
					...
					var sql = "SELECT * from master_location WHERE imei_no=? AND feeder_pillar_no=?";
					console.log('----------------------------------- '+data.imei_no+' '+data.feeder_pillar_no+' ');
					connection.query(sql, [data.imei_no, data.feeder_pillar_no], function (error, result) {
						if (error) {
					  		console.log(error);
					  		connection.release();
					  	...
					  	if(result.length > 0) {
							...
							var sql = 'UPDATE master_location SET ? , update_time = NOW() WHERE feeder_pillar_no="'+data.feeder_pillar_no+'" AND imei_no="'+data.imei_no+'"';
...
router.post('/api/city-insert', checkAuth, function(req, res, next) {
	var insertdata = req.body.data;
...
router.post('/api/city-update', checkAuth, function(req, res, next) {
	var updatedata = req.body.data;
	if(updatedata.id != ''){
		var updateq = {
			state_id: updatedata.state_id,
...
router.post('/api/city-delete', checkAuth, function(req, res, next) {
	var deleteid = req.body.deleteid;
	if(deleteid != ''){
		POOLDB[req.hostname].getConnection(function(err, connection) {
			if (err) console.log(err);
...
router.post('/api/ward-insert', checkAuth, function(req, res, next) {
	var insertdata = req.body.data;
	if(insertdata.state_id != ''){
		var insertq = {
			ulb_id: insertdata.ulb_id,
...
router.post('/api/ward-update', checkAuth, function(req, res, next) {
	var updatedata = req.body.data;
	if(updatedata.id != ''){
		var updateq = {
			ulb_id: updatedata.ulb_id,
...
router.post('/api/ward-delete', checkAuth, function(req, res, next) {
	var deleteid = req.body.deleteid;
	if(deleteid != ''){
		POOLDB[req.hostname].getConnection(function(err, connection) {
			if (err) console.log(err);
...
router.post('/api/users', checkAuth, function(req, res, next) {
	...
	POOLDB[req.hostname].getConnection(function(err, connection) {
		...
		connection.query(sql1+sql2+sql3, function (error, result) {
			connection.release();
		  	...
		  	if (error) {
				...
				res.send(JSON.stringify(mess));
		  		...
		  		var o = {'users': result[0], 'roles': result[1], 'zones': result[2]};
...
router.get('/api/getbsesrecords/lucknow', function(req, res, next) {
	...
	var sql = "SELECT ml.meter_type, ml.feeder_pillar_no, ml.meter_no as switch_id, ml.location_lat as latitude, ml.location_lng as longitude, ml.no_of_fittings, ml.location, total_kw, dld.amp_r, dld.amp_y, dld.amp_b, dld.kwh, dld.output_status, DATE_FORMAT(dld.data_stamp, '%Y-%m-%d %H:%i:%s') as data_stamp "+
