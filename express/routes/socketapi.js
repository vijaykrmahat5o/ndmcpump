...
router.get("/getsome", function (req, res) {
	...
	res.send({
		data_from_ccms_live_socket: JSON.stringify(req.body),
	});
});

const net = require("net");
const client = new net.Socket('103.211.219.21:32235');
client.on("connect", () => {
  console.log("Server Connected! sdasdasd 103.211.219.21:32235");
...
client.connect("32235", "103.211.219.21", function () {
  console.log("Connected vijay ----");
  var o = {};
  o.imei 		= '869622057813389';
  o.command 	= 'OFF';

  //console.log('------',client.emit('commtoser', o));

  var ccc = client.emit('commtoser',o);
...
router.post("/pcmcdataUp", function (req, res, next) {
  var param = req.body;

  //console.log("######## Param ######## :"+JSON.stringify(param, null, 4));

  console.log("MDMS PUNE POST DATA");
  ...
  var file = date.getUTCFullYear() + "-" + (date.getUTCMonth() + 1) + "-" + date.getUTCDate() + "-pcmc-mdms.txt";
...
router.post("/pcmcdataDownEvent", function (req, res, next) {
  var param = req.body;
  ...
  var time = date.getTime();
  const path = require("path");
  var file = date.getUTCFullYear() + "-" + (date.getUTCMonth() + 1) + "-" + date.getUTCDate() + "-pcmc-kerlink.txt";
  fs.exists(path.normalize(__dirname + "/../logs/" + file), function (exist) {
    if (exist) {
      var str = time + "," + JSON.stringify(data);
      fs.appendFile(path.normalize(__dirname + "/../logs/" + file), "\r\n" + str, function (err1) {
...
router.post("/pushdata", function (req, res, next) {
  ...
  if (!empty(param.DevEUI_uplink)) {
    ...
    hexPayload = param.DevEUI_uplink.payload_hex;
    ...
    insertq = {
      trans_id: 1,
      device_id: param.devEui,
      payload: param.pdu,
      timestamp: moment(param.txtime).format("YYYY-MM-DD HH:mm:ss"),
    };
    hexPayload = param.pdu;
  }
  ...
  /*var insertq = {
	...
	var hexPayload = param.pdu;*/

  console.log("HEX DATA: " + hexPayload);
  ...
  var time = date.getTime();
  const path = require("path");
  var file = date.getUTCFullYear() + "-" + (date.getUTCMonth() + 1) + "-" + date.getUTCDate() + "-mdms.txt";
  fs.exists(path.normalize(__dirname + "/../logs/" + file), function (exist) {
    if (exist) {
      var str = time + "," + JSON.stringify(data);
      fs.appendFile(path.normalize(__dirname + "/../logs/" + file), "\r\n" + str, function (err1) {
...
function decodeAxioma(data) {
  console.log("Decode Axioma");
  var _arr = new Array();
  //console.log(data.payload_hex.substr(0, 8));
...
router.post("/deviceonoff", function (req, res, next) {
  ...
  console.log('oooooooooooooooooooooooooooooooooooo');
    ...
    if(imei_no!='' && command!=''){
      ...
      if (socket_connected) {
          ...
          var argsString = base64encode(JSON.stringify(o));
      ...
      var mess = { error: 0, message: "Light Is "+command+" !", result: [] };
      res.send(JSON.stringify(mess));
    }else{			
      var mess = { error: 1, message: "May be IMEI and Command is wrong please check!", result: [] };
      res.send(JSON.stringify(mess));
