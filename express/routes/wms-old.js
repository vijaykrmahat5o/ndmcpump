  	...
  	var accesskey = req.body.accesskey;
	  	...
	  	pool.getConnection(function(err, connection) {
			if (err){
				...
				connection.query('SELECT u.id, u.permissions from users u WHERE u.access_key = "'+accesskey+'"', function (error, rows, fields) {
				  	connection.release();
	...
	pool.getConnection(function(err, connection) {
		if (err){
			...
			if(devicetype == 'waterlevel'){
				sql = "SELECT md.id, md.imei_no, md.uniq_no, md.mobile_no, md.device_no, ml.location, ml.longitude, ml.latitude, ml.location_type, ml.tank_height, tld.sl_voltage, IFNULL(tld.update_time, '') as update_time, tld.analoginput FROM master_device as md LEFT JOIN trans_last_data as tld ON tld.imei_no = md.imei_no LEFT JOIN master_location ml ON ml.uniq_no=md.uniq_no;";
			}else{
				sql = "SELECT md.id, md.imei_no, md.uniq_no, md.mobile_no, md.device_no, ml.location, ml.longitude, ml.latitude, ml.location_type, ml.tank_height, tld.sl_voltage, IFNULL(tld.update_time, '') as update_time, tld.analoginput FROM master_device as md LEFT JOIN trans_last_data as tld ON tld.imei_no = md.imei_no LEFT JOIN master_location ml ON ml.uniq_no=md.uniq_no;";
			}
...
router.post('/device-data', checkAuth, function(req, res, next) {
	...
	var sql = 'SELECT md.imei_no, md.uniq_no, mobile_no, ml.location, ml.longitude, ml.latitude, tld.sl_voltage, IFNULL(tld.update_time, "") as update_time, tld.analoginput FROM master_device as md LEFT JOIN trans_last_data as tld ON tld.imei_no = md.imei_no LEFT JOIN master_location ml ON ml.uniq_no=md.uniq_no WHERE md.imei_no="'+imei_no+'";';
...
router.post('/add-location', checkAuth, async function(req, res, next) {
	...
	if(insertdata.imei_no != ''){
		...
		sql += "INSERT INTO master_location (uniq_no, location, latitude, longitude, location_type, tank_height) VALUES ('"+insertdata.uniq_no+"', '"+insertdata.location+"', '"+insertdata.latitude+"', '"+insertdata.longitude+"', '"+insertdata.location_type+"', '"+JSON.stringify(insertdata.tank_height)+"');";
...
router.post('/update-location', checkAuth, function(req, res, next) {
	...
	if(updatedata.imei_no != ''){
		var sql1 = "UPDATE master_device SET mobile_no='"+updatedata.mobile_no+"', device_no='"+updatedata.device_no+"' WHERE uniq_no='"+updatedata.uniq_no+"';";

		var sql2 = "UPDATE master_location SET location_type='"+updatedata.location_type+"', location='"+updatedata.location+"', longitude='"+updatedata.longitude+"', latitude='"+updatedata.latitude+"' WHERE uniq_no='"+updatedata.uniq_no+"';";
...
router.post('/delete-location', checkAuth, function(req, res, next) {
	...
	if(deleteid != ''){
		var sql = "DELETE FROM master_location WHERE uniq_no=(SELECT uniq_no FROM master_device WHERE id='"+deleteid+"');";
		sql += "DELETE FROM trans_last_data WHERE imei_no=(SELECT imei_no FROM master_device WHERE id='"+deleteid+"');";
				...
				if (usr_err) {
			 	...
			 	// console.log('The solution is: ', rows[0].device_id)
					if(usr_rows.length > 0){
						var access_key = 10001;//new Date().getTime();
