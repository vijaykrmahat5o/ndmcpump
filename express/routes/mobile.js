...
var pool_jhansi = mysql.createPool({
    connectionLimit : 50,
    host: 'localhost',
    user: 'jhansi_usr',
    password: 'ND9VSlnbEYZr',
    database: 'jhansi_db',
    multipleStatements: true,
    dateStrings: [
      'DATE',
...
var DASHBOARD = {
	...
	'103.211.219.21': '/state-dashboard/in-jhansi'
}
...
router.get('/', function(req, res, next) {
  res.render('index', { title: 'Express Mobile API' });
});

router.get('/api/promise', function(req,res,next){
...
function checkAuth(req, res, next) {
	...
	if(req.body.length > 0){
		accesskey = req.body[0].accesskey;
			...
			connection.query(sql, function (error, result) {
				if (error) {
					...
					if(result.length > 0){
						...
						console.log('------------- MOBILE LOGIN SUCCESSFULLY ----------- '+req.user_id);
...
router.post('/get-feeder-pillar', checkAuth, function(req, res, next) {
	...
	if(fp_no > 0){
		POOLDB[req.hostname].getConnection(function(err, connection) {
			...
			var sql = 'SELECT imei_no, location, state_id, ulb_id FROM master_location WHERE feeder_pillar_no="'+fp_no+'"';
...
router.post('/api/dashboard', checkAuth, function(req, res, next) {
	...
	POOLDB[req.hostname].getConnection(function(err, connection) {
		...
		if(!empty(state_perm)){
			...
			sql += "SELECT mz.id, mz.title, mz.areacode, mc.title as city_title, mc.areacode as city_code FROM master_zone mz LEFT JOIN master_city mc ON mz.city=mc.id;";
		}
...
router.post('/api/consumption', checkAuth, function(req, res, next) {
	...
	var sql = "SELECT feeder_pillar_no, imei_no, REPLACE(location, ',', '') as location, meter_no, no_of_fittings FROM master_location WHERE ulb_id="+ulb+";";
	sql += "SELECT imei_no, kwh, meter_no, DATE_FORMAT(update_time, '%Y-%m-%d') as update_time FROM consumption_daily WHERE ulb_id="+ulb+" AND update_time >= DATE('"+datefrom+" 00:00:01') AND update_time <= DATE_ADD('"+dateto+" 00:00:01', INTERVAL 1 DAY);";
...
router.post('/api/monthlyconsumption', checkAuth, function(req, res, next) {
	...
	var sql = "SELECT total_load, feeder_pillar_no, imei_no, REPLACE(location, ',', '') as location, meter_no, no_of_fittings FROM master_location WHERE ulb_id="+ulb+";";
	sql += "SELECT imei_no, kwh, meter_no, DATE_FORMAT(update_time, '%Y-%m-%d') as update_time FROM consumption_monthly WHERE ulb_id="+ulb+" AND update_time > DATE_SUB(DATE_FORMAT(NOW() ,'%Y-%m-01'),INTERVAL 6 MONTH);";
...
router.post('/device-ulbs', checkAuth, function(req, res, next) {
	...
	var perms = req.permissions;
	console.log('MOBILE PERMISSION REQUEST URL: '+req.hostname);
		...
		connection.query(sql, function (error, result) {
			...
			var mess = {};
		  	if (error) {
		  		...
		  		if(!empty(perms.state_perm)){
					sql = "SELECT md.id, md.feeder_pillar_no, md.device_type, ml.modem_type, dld.door_status, ml.state_id, ml.zone_id, ml.ulb_id, ml.total_connection, ml.previous_load, ml.total_load, ml.location, COALESCE(dld.total_kw, 0) total_kw, dld.datalength, dld.supply, dld.amp_r, dld.amp_y, dld.amp_b, dld.kw_r, dld.kw_y, dld.kw_b, dld.kwh, dld.output_status, DATE_FORMAT(dld.data_stamp, '%Y-%m-%d %H:%i:%s') as data_stamp, ml.maintenance, DATE_FORMAT(dld.update_time, '%Y-%m-%d %H:%i:%s') as update_time, DATE_FORMAT(ml.insert_time, '%Y-%m-%d %H:%i:%s') as insert_time "+
					"FROM master_device AS md "+
					...
					sql = "SELECT md.id, md.imei_no, md.feeder_pillar_no, md.mobile_no, md.device_type, ml.modem_type, md.device_no, md.data_fault, dld.meter_no as sys_meter_no, dld.meter_fault, dld.door_status, COALESCE(md.fault_time, '') as fault_time, ml.state_id, ml.zone_id, ml.ulb_id, ml.ward_id, ml.meter_type, ml.total_connection, ml.previous_load, ml.total_load, ml.location_lat, ml.location_lng, ml.no_of_fittings, ml.location, COALESCE(dld.total_kw, 0) total_kw, dld.datalength, dld.supply, dld.amp_r, dld.amp_y, dld.amp_b, dld.kw_r, dld.kw_y, dld.kw_b, dld.kwh, dld.output_status, DATE_FORMAT(dld.data_stamp, '%Y-%m-%d %H:%i:%s') as data_stamp, dld.meter_phase_type, ml.maintenance, DATE_FORMAT(dld.update_time, '%Y-%m-%d %H:%i:%s') as update_time, DATE_FORMAT(ml.insert_time, '%Y-%m-%d %H:%i:%s') as insert_time "+
					"FROM master_device AS md "+
					"LEFT JOIN master_location AS ml ON ml.feeder_pillar_no = md.feeder_pillar_no "+
					"LEFT JOIN device_last_data AS dld ON dld.imei_no = md.imei_no "+
		  		...
		  		if(filters.search != ""){
					...
					if(!empty(perms.state_perm)){
		  			...
		  			if(!empty(perms.state_perm)){
						...
						"LEFT JOIN device_last_data AS dld ON dld.imei_no = md.imei_no "+
						"WHERE ml.ulb_id = '"+ulb_id+"' AND ml.state_id='"+perms.state_perm+"' AND ml.status!=2 LIMIT "+limit+" OFFSET "+offset+";";
					}else{
						sql = "SELECT md.id, md.imei_no, md.feeder_pillar_no, md.mobile_no, md.device_type, md.device_no, md.data_fault, dld.meter_fault, dld.door_status, COALESCE(md.fault_time, '') as fault_time, ml.state_id, ml.zone_id, ml.ulb_id, ml.ward_no, ml.meter_type, ml.total_connection, ml.previous_load, ml.total_load, ml.location_lat, ml.location_lng, ml.no_of_fittings, ml.location,  ml.led_18w, ml.led_20w, ml.led_24w, ml.led_32w, ml.led_35w, ml.led_40w, ml.led_45w, ml.led_50w, ml.led_60w, ml.led_70w, ml.led_72w, ml.led_75w, ml.led_80w, ml.led_90w, ml.led_100w, ml.led_110w, ml.led_120w, ml.led_130w, ml.led_140w, ml.led_150w, ml.led_190w, ml.led_200w, ml.led_250w, ml.led_400w, COALESCE(dld.total_kw, 0) total_kw, dld.datalength, dld.supply, dld.amp_r, dld.amp_y, dld.amp_b, dld.kwh, dld.output_status, DATE_FORMAT(dld.data_stamp, '%Y-%m-%d %H:%i:%s') as data_stamp, ml.maintenance, DATE_FORMAT(dld.update_time, '%Y-%m-%d %H:%i:%s') as update_time "+
						"FROM master_device AS md "+
						"LEFT JOIN master_location AS ml ON ml.feeder_pillar_no = md.feeder_pillar_no "+
						"LEFT JOIN device_last_data AS dld ON dld.imei_no = md.imei_no "+
						"WHERE ml.ulb_id = '"+ulb_id+"' AND ml.status!=2 LIMIT "+limit+" OFFSET "+offset+";";
					}
		  		}
	        ...
	        if(!empty(csvfile)){
			    ...
			    .on('data', (data) => {
			    	...
			    	if(!empty(data.feeder_pillar_no)){
			    		...
			    		import_result.push({'fpno':data.feeder_pillar_no, 'imei_no': data.imei_no, 'status':'No Feeder Pillar Number'});
			    	}
			    })
			    .on('end', async () => {
			    	console.log("END");
    ...
    if (!req.files || Object.keys(req.files).length === 0) {
        ...
        if(!empty(csvfile)){
			...
			//if (err) console.log(err);
			    ...
			    .pipe(csv())
			    .on('data', (data) => {
			    	console.log('>>>>'+data.feeder_pillar_no+' '+data.ulb_id+' '+data.imei_no.toString()+' '+data.mobile_no);
					...
					connection.query(sql, inputarr, function (error, result) {
						if (error) {
					  	...
					  	if(result.length > 0) {
							...
							if(!empty(data.imei_no) && !empty(data.feeder_pillar_no)){
								sql = 'UPDATE master_location SET ? , update_time = NOW() WHERE feeder_pillar_no="'+data.feeder_pillar_no+'" AND imei_no="'+data.imei_no+'"';
							}else if(!empty(data.feeder_pillar_no)){
								sql = 'UPDATE master_location SET ? , update_time = NOW() WHERE feeder_pillar_no="'+data.feeder_pillar_no+'"';
							}else if(!empty(data.imei_no)){
...
router.post('/api/zone-record', checkAuth, function(req, res, next) {
	var sql = "SELECT id, state, city, title, title as ulb, areacode, location, parent, zoom, previous_load, tariff_rate, on_time, off_time, sms_mobile, status FROM master_zone WHERE parent != 0 ORDER BY title ASC";
...
router.post('/api/device-record', checkAuth, function(req, res, next) {
	...
	if(imei_no != ''){
		POOLDB[req.hostname].getConnection(function(err, connection) {
			...
			var sql = 'SELECT datalength, supply, voltage_r, voltage_y, voltage_b, amp_r, amp_y, amp_b, kw_r, kw_y, kw_b, kwh, total_kw, update_time, door_status, meter_phase_type, output_status FROM device_received_data WHERE imei_no=? ORDER BY id DESC LIMIT 500;';
...
router.post('/api/device-report', checkAuth, function(req, res, next) {
	...
	if(data.imei_no != '' || data.feeder_pillar_no != ''){
		POOLDB[req.hostname].getConnection(function(err, connection) {
			...
			var sql = 'SELECT datalength, supply, voltage_r, voltage_y, voltage_b, amp_r, amp_y, amp_b, kw_r, kw_y, kw_b, kwh, total_kw, update_time, door_status, meter_phase_type, output_status FROM device_received_data_clone WHERE imei_no=? AND update_time>=? AND update_time<=? ORDER BY id DESC LIMIT 3000;';

			if(data.feeder_pillar_no != ''){
				condition = data.feeder_pillar_no;
				sql = 'SELECT datalength, supply, voltage_r, voltage_y, voltage_b, amp_r, amp_y, amp_b, kw_r, kw_y, kw_b, kwh, total_kw, update_time, door_status, meter_phase_type, output_status FROM device_received_data_clone WHERE imei_no=(SELECT imei_no FROM master_location WHERE feeder_pillar_no=?) AND update_time>=? AND update_time<=? ORDER BY id DESC LIMIT 3000;';
			}
			
			var q = connection.query(sql, [condition, data.start_date+' 00:00:00', data.end_date+' 23:59:59'], function (error, result) {
				connection.release();
...
router.post('/api/user-update', checkAuth, saveUserActivity, function(req, res, next) {
	...
	if(updatedata.id != ''){
		...
		var sql = "UPDATE users SET username='"+updatedata.username+"', email='"+updatedata.email+"', "+password+" role='"+updatedata.role+"', secure_code='"+updatedata.secure_code+"', permissions='"+JSON.stringify(updatedata.permissions)+"', status='"+updatedata.status+"' WHERE id='"+updatedata.id+"';";
...
router.post('/api/complaint-record', checkAuth, function(req, res, next) {
	...
	if(ulbareacode != ''){
		...
		var sql = 'SELECT c.*, mz.title ulb_title, ml.location location_title FROM complaints c LEFT JOIN master_zone mz ON mz.id=c.ulb LEFT JOIN master_location ml ON ml.feeder_pillar_no = c.location ORDER BY id DESC LIMIT 50;';
		if(!empty(perms.ulb_perm)){
			sql = 'SELECT c.*, ml.location location_title FROM complaints c LEFT JOIN master_zone mz ON mz.id=c.ulb LEFT JOIN master_location ml ON ml.feeder_pillar_no = c.location WHERE c.state="'+perms.state_perm+'" AND mz.areacode="'+perms.ulb_perm+'" ORDER BY c.id DESC LIMIT 50;';
		}
...
router.post('/api/dailyreports-generate', checkAuth, function(req, res, next) {
	...
	if(insertdata.imei_no != ''){
		...
		var filename = 'report-imei-'+insertdata.imei_no+'-'+insertdata.start_date+'-'+insertdata.end_date+'-'+unique_id;

		var header = ['SUPPLY','VOLTAGE R','VOLTAGE Y','VOLTAGE B','AMP R','AMP Y','AMP B','KW R','KW Y','KW B','KWH','TOTAL KW','UPDATE TIME','DOOR STATUS','OUTPUT STATUS'];
...
"FROM device_received_data_clone drd WHERE imei_no='"+imei_no+"' AND data_stamp >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND data_stamp <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') ORDER BY id DESC LIMIT 2000 ;";
		...
		var filename = 'report-fp-'+insertdata.feeder_pillar_no+'-'+insertdata.start_date+'-'+insertdata.end_date+'-'+unique_id;
...
router.post('/api/reports-generate', checkAuth, function(req, res, next) {
	...
	if(type == "OVERVOLTAGE"){
		...
		var header = ['DISTRICT', 'ULB', 'IMEI','FEEDER PILLAR','VOLTAGE R','VOLTAGE Y','VOLTAGE B','START TIME','END TIME'];
		...
		new Promise(function(resolve, reject) {
			...
			POOLDB[req.hostname].getConnection(function(err, connection) {
		  	...
		  	new Promise(function(resolve, reject) {
		  		var sql = "SELECT mz2.title, mz.title, ov.imei_no, ov.fp_no, if(ov.voltage_r/100 < 80,0,ov.voltage_r/100) as voltage_r, if(ov.voltage_y/100 < 80,0,ov.voltage_y/100) as voltage_y, if(ov.voltage_b/100 < 80,0,ov.voltage_b/100) as voltage_b, ov.start_time, ov.end_time FROM report_overvoltage ov LEFT JOIN master_zone mz ON ov.ulb=mz.id LEFT JOIN master_zone mz2 ON ov.district=mz2.id WHERE ov.start_time >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.end_time <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') AND mz.id='"+insertdata.ulb+"' AND (ov.voltage_r/100 < 440 && ov.voltage_y/100 < 440 && ov.voltage_b/100 < 440) ORDER BY ov.id DESC LIMIT 300;";
		  		process_start_time = new Date().getTime();
		...
		var header = ['DISTRICT', 'LOCATION', 'FEEDER PILLAR', 'TOTAL LOAD (KW)','START TIME','START LOAD (KW)','END TIME','END LOAD (KW)'];
		...
		new Promise(function(resolve, reject) {
			...
			POOLDB[req.hostname].getConnection(function(err, connection) {
		  	...
		  	new Promise(function(resolve, reject) {
		  		var sql = "SELECT  mz2.title, REPLACE(ml.location, ',', '') as location, ov.fp_no, (ml.total_load/1000) as total_load, ov.start_time, (ov.start_load/100000) as start_load, ov.end_time, (ov.end_load/100000) as end_load FROM report_overload ov LEFT JOIN master_zone mz ON ov.ulb=mz.id LEFT JOIN master_zone mz2 ON ov.district=mz2.id LEFT JOIN master_location ml ON ml.feeder_pillar_no = ov.fp_no WHERE ml.total_load < (ov.start_load/100) AND ov.start_time >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.end_time <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') AND mz.id='"+insertdata.ulb+"' ORDER BY ov.id DESC LIMIT 300 ;";
		...
		var sql="";
		if(insertdata.ulb != ''){
			sql = "SELECT 'ULB', 'DEVICES','DEVICES LOSS','CONTROLLERS','CONTROLLER ON','CONTROLLER OFF','CONTROLLER LOSS', 'POWER CUT', 'TOTAL LOAD', 'ACTUAL LOAD', 'TOTAL KWH', 'UPDATE TIME' UNION ALL SELECT mz.title, ov.total_device, ov.total_device_loss, ov.total_controller, ov.total_controller_on, ov.total_controller_off, ov.total_controller_loss, ov.total_powercut, ov.total_load, ov.actual_load, ov.total_kwh, ov.update_time FROM report_ulb_data ov LEFT JOIN master_zone mz ON ov.ulb_id=mz.id WHERE ov.update_time >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.update_time <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') AND ov.ulb_id='"+insertdata.ulb+"' ORDER BY ov.id DESC LIMIT 300 INTO OUTFILE '"+mysqlpath+"' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';";
		}else{
			sql = "SELECT 'ULB', 'DEVICES','DEVICES LOSS','CONTROLLERS','CONTROLLER ON','CONTROLLER OFF','CONTROLLER LOSS', 'POWER CUT', 'TOTAL LOAD', 'ACTUAL LOAD', 'TOTAL KWH', 'UPDATE TIME' UNION ALL SELECT mz.title, ov.total_device, ov.total_device_loss, ov.total_controller, ov.total_controller_on, ov.total_controller_off, ov.total_controller_loss, ov.total_powercut, ov.total_load, ov.actual_load, ov.total_kwh, ov.update_time FROM report_ulb_data ov LEFT JOIN master_zone mz ON ov.ulb_id=mz.id WHERE ov.update_time >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.update_time <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') AND mz.id='"+insertdata.ulb+"' ORDER BY ov.id DESC LIMIT 300 INTO OUTFILE '"+mysqlpath+"' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n';";
		}
		var ulb = '';
		conn.query(sql, function (err, result, fields) {
			if (err) {
...
router.post('/api/ulb-report', checkAuth, function(req, res, next) {
	...
	POOLDB[req.hostname].getConnection(function(err, connection) {
		...
		var sql = "SELECT mz.title, ov.total_device, ov.total_device_loss, ov.total_controller, ov.total_controller_on, ov.total_controller_off, ov.total_controller_loss, ov.total_powercut, ov.total_load, ov.actual_load, ov.total_kwh, ov.update_time FROM report_ulb_data ov LEFT JOIN master_zone mz ON ov.ulb_id=mz.id WHERE ov.update_time >= DATE_FORMAT('"+data.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.update_time <= DATE_FORMAT('"+data.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') AND ov.ulb_id='"+data.ulb+"' ORDER BY ov.id DESC LIMIT 500";
...
router.post('/task-added', checkAuth, function(req, res, next) {
	var insertdata = JSON.parse(req.body.data);
...
router.post('/api/task-record', checkAuth, function(req, res, next) {
	var sql = 'SELECT t.*, u.username as username FROM tasks t LEFT JOIN users u ON u.id = t.user_id WHERE insert_time > now() - INTERVAL 7 day ORDER BY id DESC; ';
...
router.post('/user-attendance', checkAuth, function(req, res, next) {
	var insertdata = req.body;

	console.log(JSON.stringify(insertdata));
	
	if(insertdata.latitude != ''){
		var insertq = {
			user_id: req.user_id,
		...
		var sql = 'INSERT INTO user_attendance SET ? , insert_time = NOW() ';
...
router.post('/user-tracking', checkAuth, function(req, res, next) {
	var user_id = req.user_id;
	var insertdata = req.body;
	...
	if(insertdata.latitude != ''){
		...
		var sql = 'INSERT INTO user_tracking SET ? , insert_time = NOW(); UPDATE master_tracking SET ?, update_time = NOW() WHERE user_id=?; ';
		...
		pool_login.getConnection(function(err, connection) {
			if (err) {
				...
				var query = connection.query(sql, [insertq, updateq, user_id], function (error, result) {
					connection.release();
...
router.post('/user-tracking2', checkAuth, function(req, res, next) {
	var user_id = req.user_id;
	var insertdata = req.body;
	console.log(insertdata);
		...
		var insertq = {
			user_id: req.user_id,
			latitude: insertdata.location.coords.latitude,
			longitude: insertdata.location.coords.longitude,
			data: JSON.stringify(insertdata)
		}
		...
		var sql = 'INSERT INTO user_tracking_2 SET ? , insert_time = NOW(); ';
