...
router.post('/api/reports-generate', checkAuth, function(req, res, next) {
	...
	if(type == "OVERVOLTAGE"){
		...
		let promise = new Promise(function(resolve, reject) {
			...
			console.log('>>>> ');
			var sql = "SELECT  mz2.title, REPLACE(ml.location, ',', ''), ov.fp_no, (ml.total_load/1000), ov.start_time, (ov.start_load/100000), ov.end_time, (ov.end_load/100000) FROM report_overload ov LEFT JOIN master_zone mz ON ov.ulb=mz.id LEFT JOIN master_zone mz2 ON ov.district=mz2.id LEFT JOIN master_location ml ON ml.feeder_pillar_no = ov.fp_no WHERE ml.total_load < (ov.start_load/100) AND ov.start_time >= DATE_FORMAT('"+insertdata.start_date+" 00:00:00', '%y-%m-%d %H:%i:%s') AND ov.end_time <= DATE_FORMAT('"+insertdata.end_date+" 23:59:59', '%y-%m-%d %H:%i:%s') AND mz.id='"+insertdata.ulb+"' ORDER BY ov.id DESC LIMIT 300 ;";
...
router.post('/api/devicecorrection-update', checkAuth, function(req, res, next) {
	...
	if(type == 'location'){
		if(updatedata.imei_no != ''){
			...
			conn.query(sql, location, function (err, result, fields) {
			  	if (err) {
			  		console.log(err);
			  	}                  
			 	var mess = {'error':0,'message':'Device Updated Successfully!','result':1};
		...
		conn.query(sql, updatedata.imei_no, function (err, result, fields) {
		  	if (err) {
		  		console.log(err);
		  	}                  
		 	var mess = {'error':0,'message':'Device Updated Successfully!','result':1};
