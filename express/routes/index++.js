...
router.post('/api/ulb-devices', checkAuth, function(req, res, next) {
	...
	"LEFT JOIN device_last_data AS dld ON dld.imei_no = md.imei_no "+
	"WHERE mz.areacode = '"+ulbcode+"' AND ml.state_id='"+perms.state_perm+"';";
	sql += "SELECT count(md.imei_no) as total_count FROM master_device AS md "+
	"LEFT JOIN master_location AS ml ON ml.feeder_pillar_no = md.feeder_pillar_no "+
	"LEFT JOIN master_zone AS mz ON mz.id = ml.ulb_id "+
	"WHERE mz.areacode = '"+ulbcode+"' AND ml.state_id='"+perms.state_perm+"';";
	sql += "SELECT * FROM master_zone WHERE areacode='"+ulbcode+"' AND state='"+perms.state_perm+"';";
	...
	"LEFT JOIN master_zone AS mz ON mz.id = ml.ulb_id "+
	"LEFT JOIN device_last_data AS dld ON dld.imei_no = md.imei_no "+
	"WHERE mz.areacode = '"+ulbcode+"' LIMIT "+limit+" OFFSET "+offset+";";
	sql += "SELECT count(md.imei_no) as total_count FROM master_device AS md "+
	"LEFT JOIN master_location AS ml ON ml.feeder_pillar_no = md.feeder_pillar_no "+
	"LEFT JOIN master_zone AS mz ON mz.id = ml.ulb_id "+
	"WHERE mz.areacode = '"+ulbcode+"';";
...
router.post('/api/device-update', checkAuth, function(req, res, next) {
	...
	if(updatedata.imei_no != ''){
		...
		sql += "UPDATE master_location SET ? WHERE feeder_pillar_no='"+updatedata.feeder_pillar_no+"';";

		conn.query(sql, [device, location], function (err, results, fields) {
		  	if (err) {
		  		console.log(err);
		  	}                  
...
router.post('/api/device-record', checkAuth, function(req, res, next) {
	...
	if(imei_no != ''){
		pool.getConnection(function(err, connection) {
			...
			var sql = 'SELECT datalength, supply, voltage_r, voltage_y, voltage_b, amp_r, amp_y, amp_b, kw_r, kw_y, kw_b, kwh, total_kw, update_time, door_status, meter_phase_type, output_status FROM device_received_data WHERE imei_no=? ORDER BY id DESC LIMIT 100;';
