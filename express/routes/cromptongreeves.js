...
var DASHBOARD = {
	'crompton.ccms.live': '/state-dashboard/in-mh'
}
var STATEIP = {
...
router.post('/api/consumption', checkAuth, function(req, res, next) {
	...
	var sql = "SELECT feeder_pillar_no, IFNULL(consumer_no, '') as consumer_no, imei_no, REPLACE(location, ',', '') as location, meter_no, no_of_fittings FROM master_location WHERE ulb_id="+ulb+";";
...
router.post('/api/monthlyconsumption', checkAuth, function(req, res, next) {
	...
	var sql = "SELECT ml.total_load, zo.title as zone, IFNULL(ml.consumer_no, '') as consumer_no, ml.feeder_pillar_no, ml.imei_no, REPLACE(ml.location, ',', '') as location, ml.meter_no, ml.no_of_fittings, ml.led_18w, ml.led_20w, ml.led_24w, ml.led_28w, ml.led_32w, ml.led_35w, ml.led_40w, ml.led_45w, ml.led_50w, ml.led_60w, ml.led_70w, ml.led_72w, ml.led_75w, ml.led_80w, ml.led_90w, ml.led_96w, ml.led_100w, ml.led_110w, ml.led_120w, ml.led_130w, ml.led_140w, ml.led_150w, ml.led_190w, ml.led_200w, ml.led_250w, ml.led_400w FROM master_location ml LEFT JOIN zones as zo ON zo.ulb_id = ml.ulb_id AND zo.id=ml.zone_id WHERE ml.ulb_id="+ulb+";";
	...
	sql += "SELECT imei_no, kwh, meter_no, DATE_FORMAT(update_time, '%Y-%m-%d') as update_time FROM consumption_monthly WHERE ulb_id="+ulb+" AND update_time > DATE_SUB(DATE_FORMAT(NOW() ,'%Y-%m-01'),INTERVAL 8 MONTH);";
				...
				if(usr_result.length > 0){
					...
					pool_login.getConnection(function(err, connection) {
						...
						connection.query(sql, function (perm_err, perm_result) {
							...
							if(!empty(permissions.state_perm)){
								console.log(">>>> "+permissions.state_perm.replace("-","_").toUpperCase()+" ");
							}
							//console.log(">>>> "+permissions.state_perm.replace("-","_").toUpperCase()+" ");
							...
							var o = {};
								...
								if(!empty(permissions.state_perm)){
									o.ip_addr = STATEIP[permissions.state_perm.replace("-","_").toUpperCase()];
								}else{
									o.ip_addr = '';
								}
...
router.post('/api/task-record', checkAuth, function(req, res, next) {
	var sql = 'SELECT t.*, u.username as username FROM tasks t LEFT JOIN users u ON u.id = t.user_id WHERE insert_time > now() - INTERVAL 30 day ORDER BY id DESC; ';
...
router.post('/api/user-tracking', checkAuth, function(req, res, next) {
	var sql = 'SELECT latitude lat, longitude lng, data FROM user_tracking_2 WHERE user_id=135 AND insert_time >= DATE_FORMAT("2021-10-28 17:00:00", "%y-%m-%d %H:%i:%s") AND insert_time <= DATE_FORMAT("2021-10-28 23:59:59", "%y-%m-%d %H:%i:%s")';
	pool_login.getConnection(function(err, connection) {
		if (err) {
			console.log(err);
			var mess = {'error':1,'message':'Database connection error!','result':[]};
