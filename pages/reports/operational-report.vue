                ...
                <b-row class="pb-2">
                    <b-col cols="auto">
                        <b-breadcrumb class="mb-0">
                            ...
                            <b-breadcrumb-item active>Individual Operational Report</b-breadcrumb-item>
                        </b-breadcrumb>
                    </b-col>
                    <b-col class="text-right">
                        <download-excel class="btn btn-dark btn-sm rounded-pill" :data="json_data" :fields="json_fields" header="Individual Operation Report" :subheader="excel_subheader" :before-generate="startDownloadExcel" worksheet="REPORT" name="individual-operation-report.xls">
                            Download XLS
                        </download-excel>
...
export default {
    ...
    data() {
        return {
            ...
            recordUrl: process.env.apiUrl+'individual-operational-report',
    ...
    methods: {
        filterLocation: function () {
			...
			return self.devices.filter(function (device) {
				return device.imei_no  == self.deviceid;
			})[0].location;
		},
        loadInit(){
            var self = this;
            ...
            .then(function (response) {
                ...
                if(response.data.error == 0 || response.data.error == '0'){
                    ...
                    list.forEach(item => {
                        ...
                        item.operation_date = self.$moment(item.operation_date).format('DD-MM-YYYY');
                        
                        self.json_data.push({sno: item.sno, operation_date: item.operation_date, power_hrs: item.power_hrs, failure_hrs: item.failure_hrs, on_hrs: item.on_hrs, off_hrs: item.off_hrs, auto_mode_hrs: item.auto_mode_hrs, manual_mode_hrs: item.manual_mode_hrs, trip_mode_hrs: item.trip_mode_hrs, consumption: item.consumption});
                    });
        ...
        startDownloadExcel(){
            var self = this;
            this.excel_subheader = ["Location: "+self.filterLocation(), "From: "+self.$moment(self.datefrom).format('DD-MM-YYYY')+" To:"+self.$moment(self.dateto).format('DD-MM-YYYY')]
        },
		downloadPDF(){
			var self = this;
			...
			var docDefinition = {
				content: [
                    ...
                    { text: 'Location: '+self.filterLocation(), style: 'subheading' },
                    { text: "From: "+self.$moment(self.datefrom).format('DD-MM-YYYY')+" To:"+self.$moment(self.dateto).format('DD-MM-YYYY'), style: 'subheading' },
                    {
			...
			pdfMake.createPdf(docDefinition).download("individual-operation-report-"+uniq+".pdf");
		},
    }
