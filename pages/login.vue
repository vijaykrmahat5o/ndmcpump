<template>
  <div class="login-page relative flex items-top justify-center min-h-screen bg-gray items-center sm:pt-0">
    <div class="max-w-4xl mx-auto px-8">
        ...
        <div class="mt-8 bg-white overflow-hidden shadow rounded container">
            <b-row>
                <b-col class="left-side bg-blue p-3" lg="12">
                  <h4 class="heading">Remote Monitoring & Control System</h4>
                </b-col>
                <b-col class="right-side p-6" lg="12">
                    <h4>Login to Dashboard</h4>
                    <form ref="form" class="mt-3" @submit.prevent="userLogin">
                        <b-form-group :state="userinfoState" label="Username" label-for="mobile-input" invalid-feedback="Username should be valid">
                            <b-form-input type="text" id="mobile-input" v-model="login.userinfo" :state="userinfoState"></b-form-input>
                        </b-form-group>
                        <b-form-group :state="passwordState" label="Password" label-for="password-input" invalid-feedback="Password should be greater than 4 digits">
                            <b-form-input type="password" id="password-input" v-model="login.password" :state="passwordState"></b-form-input>
                        </b-form-group>
...
export default {
    ...
    methods: {
        checkFormValidity() {
            var valid = true;
            if(this.login.userinfo.length < 4){
                valid = false;
        ...
        userLogin() {
            ...
            this.$store.dispatch('authenticateUser', {
               ...
               /*  self.$bvToast.toast('>>>>', {
                ...
                self.$router.push(self.$store.getters.getUserinfo.dashboard);
                console.log("RRRRRRRRRR::",self.$store.getters.getUserinfo.dashboard);
            });
        },
        handleSubmit(){
			if (!this.checkFormValidity()) {
            ...
            /*self.$axios.post(self.url, o).then(function (response) {
				if(response.data.error == 0 || response.data.error == '0'){
                    ...
                    self.$store.commit('setUserinfo', response.data.result);
